...
import edu.emory.mathcs.backport.java.util.Collections;
 ...
 class ReleaseServiceTest {
     ...
     public void getReleasesByArtistIds_shouldCallRepositoryfindAllByArtistIdIn_whenCalled() {
         ...
         List<String> artists = Collections.singletonList("63fa261307b34e2e2590bfe8");
         List<Release> releases = Collections.singletonList(Release.builder().artist(List.of(artist)).build());
         Mockito.when(repository.findAll(Mockito.any(Predicate.class))).thenReturn(releases);

         service.getReleasesByArtistIds(artists);

         Mockito.verify(repository, Mockito.times(1)).findAll(Mockito.any(Predicate.class));
     }

     @Test
     public void getReleasesByArtistIds_shouldReturnListOfRelease_whenCalled() {
         Artist artist = Artist.builder().id("63fa261307b34e2e2590bfe8").build();
         Artist artist2 = Artist.builder().id("63fa261307b34e2e2590bfe9").build();
         List<Artist> artists = List.of(artist, artist2);
         List<Release> releases = Collections.singletonList(Release.builder().title("Feels").artist(artists).build());
         Mockito.when(repository.findAll(Mockito.any(Predicate.class))).thenReturn(releases);

         Map<Artist, List<Release>> releasesByArtistIds = service.getReleasesByArtistIds(List.of("63fa261307b34e2e2590bfe8", "63fa261307b34e2e2590bfe9"));

         Assertions.assertNotNull(releasesByArtistIds);
         Assertions.assertEquals(2, releasesByArtistIds.size());
         Assertions.assertTrue(releasesByArtistIds.get(artist).get(0).getTitle().contains("Feels"));
         Assertions.assertTrue(releasesByArtistIds.get(artist2).get(0).getTitle().contains("Feels"));
         Assertions.assertEquals(1, releasesByArtistIds.get(artist).size());
     }
 }